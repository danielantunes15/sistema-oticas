// js/modules/vendas.js - Sistema de PDV Completo
// (Antigo js/vendas.js)
...
class VendasManager {
    ...
    adicionarAoCarrinho(produtoId) {
        ...
        if (itemExistente) {
            if (itemExistente.quantidade >= produto.estoque_atual) {
                showError('Estoque insuficiente!'); // Usa global
                return;
            }
    ...
    atualizarQuantidade(index, novaQuantidade) {
        ...
        if (novaQuantidade > item.estoque_disponivel) {
            showError('Quantidade maior que estoque disponível!'); // Usa global
            return;
        }
    ...
    async finalizarVenda() {
        if (this.carrinho.length === 0) {
            showError('Adicione produtos ao carrinho!'); // Usa global
            return;
        }
        ...
        try {
            ...
            showSuccess(`Venda #${venda.numero_venda} finalizada com sucesso!`); // Usa global
            this.limparCarrinho();

        } catch (error) {
            console.error('Erro ao finalizar venda:', error);
            showError('Erro ao finalizar venda: ' + error.message); // Usa global
        }
    }
